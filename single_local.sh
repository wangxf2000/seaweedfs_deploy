...
systemctl stop weed-mount-server.service
rm -rf /lib/systemd/system/weed*
rm -rf /usr/local/bin/weed
rm -rf /usr/local/go
rm -rf /seaweedfs
rm -rf /etc/seaweedfs
rm -rf /usr/local/certstrap
...
rm -rf /usr/local/bin/linux_amd64_full.tar.gz
mkdir -p /usr/local/bin 
tar -zxf ~/linux_amd64_full.tar.gz  -C /usr/local/bin
...
tar -xf  ~/certstrap.tar  -C /usr/local/
#进入
